...
class MyDES:
    ...
    def _bit_decode(s: list):
        return bytes(int(b, 2) for b in s).decode('utf-8')
    ...
    def _init_xor_block(self, block: str, last_block: str):
        """ Выполнить XOR начального состояния на блоке """
        if last_block == "":
            last_block = "1011111011010000101100011101000110001001110100001011010111010000"
        return self._not_or(block, last_block)
    ...
    def encode(self, enter: str, key: str):
        ...
        for block in blocks:
            # Выполняем xor блока с предыдущим
            ixb_result = self._init_xor_block(block, chained_block)
            # Выполняем замену начального состояния для каждого блока
            irb_result = self._init_replace_block(ixb_result)
            ...
            # сохраняем состояние текущего блока
            chained_block = block_result
    ...
    def decode(self, cipher_text: str, key: str):
        ...
        for block in blocks:
            ...
            block_xored_result = self._end_replace_block(block_result)
            block_result = self._init_xor_block(block_xored_result, chained_block)
            chained_block = block
...
parser: ArgumentParser = ArgumentParser(prog='mdes_chiper.py')
...
if __name__ == '__main__':
    ...
    if args.mode == "encrypt":
        ...
        encrypted_message = md.encode(message_str, key)

        print("Зашифрованный текст: ", encrypted_message)

        if args.output != "":
            save_keys(args.output, key)
        else:
        ...
        decrypted_message = md.decode(message_str, key)
